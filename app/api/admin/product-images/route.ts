...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "An error occurred while getting product images" }, { status: 500 })
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const isTemporaryProduct = productId === "temp"
    const productIdNum = isTemporaryProduct ? 0 : Number(productId)
    ...
    if (!success) {
      return NextResponse.json({ error: "Failed to add images to product" }, { status: 500 })
    }
    ...
    return NextResponse.json({ error: "An error occurred while uploading product images" }, { status: 500 })
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const success = await ProductImageService.setProductImageAsPrimary(Number(productId), imageUrl)

    if (!success) {
      return NextResponse.json({ error: "Failed to set image as primary" }, { status: 500 })
    }
    ...
    return NextResponse.json({ error: "An error occurred while setting image as primary" }, { status: 500 })
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const success = await ProductImageService.deleteProductImageByUrl(Number(productId), imageUrl)
    ...
    return NextResponse.json({ error: "An error occurred while deleting product image" }, { status: 500 })
  }
}
